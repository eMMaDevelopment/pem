...
module.exports.getPrivateKeyFromPEM = getPrivateKeyFromPEM;
module.exports.getCertFromPEM = getCertFromPEM;
...
/**
 * Returns the private key defined on a PEM file
 *
 * @param {String} certificate PEM certificate that contains private key
 * @param {Object} [options] object of cipher and optional client key password {cipher:'aes128', clientKeyPassword: 'xxx'}
 */
function getPrivateKeyFromPEM(certificate, options) {
    var keyBundle = getKeyBundle(certificate);
    return keyBundle.key;
}

/**
 * Exports the cert from a PEM File
 *
 * @param certificate certificate contined in a PEM file
 * @param {Object} [options] object of cipher and optional client key password {cipher:'aes128', clientKeyPassword: 'xxx'}
 */
function getCertFromPEM(certificate, options) {
    var keyBundle = getKeyBundle(certificate);
    return keyBundle.cert;
}
...
function readPkcs12(bufferOrPath, options, callback) {
  ...
  execBinaryOpenSSL(args, tmpfiles, function (error, stdout) {
    ...
    if (!error) {
        keybundle = getKeyBundle(stdout, options);
    }
...
/**
 * Extracts info from a PEM file
 *
 * @param stdout string containing the PEM file
 * @returns {Object} with key
 */
function getKeyBundle(stdout, options) {
    var keybundle = {};
